...
class ACTSmoothPolicy(PreTrainedPolicy):
    ...
    def forward(self, batch: dict[str, Tensor]) -> tuple[Tensor, dict]:
        ...
        # Boundary continuity loss: penalizes discontinuity at prefix->chunk boundary.
        # For each delay d, the last prefix action is at index (length_prefix_past + d - 1)
        # and the first predicted action is actions_hat[:, d-1, 0, :].
        #
        # C0: position continuity — first predicted action should match last prefix action
        # C1: velocity continuity — velocity entering boundary should match velocity leaving
        if self.config.weight_loss_boundary > 0:
            actions_full = batch[ACTION]  # [B, K+D+C, action_dim]

            # For each delay d (1..D), last prefix action is at absolute index K + d - 1
            # actions_hat[:, d_idx, 0, :] is first predicted action for delay d
            # where d_idx = d - 1 (0-indexed)
            loss_boundary_c0 = torch.tensor(0.0, device=actions_hat.device)
            loss_boundary_c1 = torch.tensor(0.0, device=actions_hat.device)
            cnt_valid = 0

            for d_idx in range(length_prefix_future):
                d = d_idx + 1  # delay value
                idx_last_prefix = length_prefix_past + d - 1

                # Last prefix action (ground truth)
                action_last_prefix = actions_full[:, idx_last_prefix]  # [B, action_dim]
                # First predicted action
                action_first_pred = actions_hat[:, d_idx, 0]  # [B, action_dim]

                # C0: position continuity
                loss_boundary_c0 = loss_boundary_c0 + F.l1_loss(action_first_pred, action_last_prefix)

                # C1: velocity continuity
                # Velocity before boundary: action_last_prefix - action_before_last_prefix
                if idx_last_prefix > 0:
                    action_before_last_prefix = actions_full[:, idx_last_prefix - 1]
                    velocity_before = action_last_prefix - action_before_last_prefix

                    # Velocity after boundary: first_pred - last_prefix
                    velocity_after = action_first_pred - action_last_prefix

                    loss_boundary_c1 = loss_boundary_c1 + F.l1_loss(velocity_after, velocity_before)

                cnt_valid += 1

            if cnt_valid > 0:
                loss_boundary_c0 = loss_boundary_c0 / cnt_valid
                loss_boundary_c1 = loss_boundary_c1 / cnt_valid

            loss_boundary = loss_boundary_c0 + loss_boundary_c1
            loss_dict["loss_boundary_c0"] = loss_boundary_c0.item()
            loss_dict["loss_boundary_c1"] = loss_boundary_c1.item()
            loss_dict["loss_boundary"] = loss_boundary.item()
            loss = loss + self.config.weight_loss_boundary * loss_boundary
