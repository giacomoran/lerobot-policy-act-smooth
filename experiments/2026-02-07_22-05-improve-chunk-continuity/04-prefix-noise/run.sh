#!/bin/bash
# 04-prefix-noise: Add Gaussian noise to action prefix embeddings during training.
...
echo "=== 04-prefix-noise: Fine-tune with prefix noise ==="
...
python "${DIR_ROOT}/scripts/eval/eval_offline_replay.py" \
    ...
    --path_output="${DIR_EXPERIMENT}/data/prefix_noise"

# Copy results
echo "=== Results ===" > "${DIR_EXPERIMENT}/report.md"
echo "" >> "${DIR_EXPERIMENT}/report.md"
echo '```' >> "${DIR_EXPERIMENT}/report.md"
cat "${DIR_EXPERIMENT}/data/prefix_noise_results.txt" >> "${DIR_EXPERIMENT}/report.md"
echo '```' >> "${DIR_EXPERIMENT}/report.md"
