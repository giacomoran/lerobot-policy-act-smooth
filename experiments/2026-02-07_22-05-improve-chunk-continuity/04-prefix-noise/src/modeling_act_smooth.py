    ...
    def forward(self, batch: dict[str, Tensor]) -> tuple[Tensor, tuple[Tensor, Tensor] | tuple[None, None]]:
        ...
        if is_training_batch:
            ...
            if length_prefix_past > 0:
                ...
                # Add Gaussian noise to prefix embeddings during training
                if self.training and self.config.std_noise_prefix > 0:
                    noise_past = torch.randn_like(embed_prefix_past) * self.config.std_noise_prefix
                    # Only add noise to non-padded positions
                    embed_prefix_past = embed_prefix_past + noise_past * (~is_pad_prefix_past.unsqueeze(-1))
            ...
            # Add Gaussian noise to prefix embeddings during training
            if self.training and self.config.std_noise_prefix > 0:
                noise_future = torch.randn_like(embed_prefix_future) * self.config.std_noise_prefix
                # Only add noise to non-padded positions
                embed_prefix_future = embed_prefix_future + noise_future * (~is_pad_prefix_future.unsqueeze(-1))
