#!/bin/bash
# 05-best-combination: Combine top-performing modifications.
# Fine-tune from step 50003 for 5000 more steps.
set -e
...
echo "=== 05-best-combination: Fine-tune with best combination ==="
...
python "${DIR_ROOT}/scripts/eval/eval_offline_replay.py" \
    ...
    --path_output="${DIR_EXPERIMENT}/data/best_combination"

# Copy results
echo "=== Results ===" > "${DIR_EXPERIMENT}/report.md"
echo "" >> "${DIR_EXPERIMENT}/report.md"
echo '```' >> "${DIR_EXPERIMENT}/report.md"
cat "${DIR_EXPERIMENT}/data/best_combination_results.txt" >> "${DIR_EXPERIMENT}/report.md"
echo '```' >> "${DIR_EXPERIMENT}/report.md"
