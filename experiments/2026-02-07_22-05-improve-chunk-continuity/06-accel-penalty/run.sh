#!/bin/bash
# 06-accel-penalty: Add L2 acceleration penalty on predicted action chunks.
...
echo "=== 06-accel-penalty: Fine-tune with acceleration penalty ==="
...
python "${DIR_ROOT}/scripts/eval/eval_offline_replay.py" \
    ...
    --path_output="${DIR_EXPERIMENT}/data/accel_penalty"

# Copy results
echo "=== Results ===" > "${DIR_EXPERIMENT}/report.md"
echo "" >> "${DIR_EXPERIMENT}/report.md"
echo '```' >> "${DIR_EXPERIMENT}/report.md"
cat "${DIR_EXPERIMENT}/data/accel_penalty_results.txt" >> "${DIR_EXPERIMENT}/report.md"
echo '```' >> "${DIR_EXPERIMENT}/report.md"
