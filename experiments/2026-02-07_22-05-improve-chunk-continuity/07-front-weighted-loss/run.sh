#!/bin/bash
# 07-front-weighted-loss: Higher L1 weight at chunk start (near boundary).
...
echo "=== 07-front-weighted-loss: Fine-tune with front-weighted loss ==="
...
lerobot-train \
    ...
    --batch_size=8 \
    --num_workers=4 \
    --policy.weight_front_emphasis=1.0 \
...
python "${DIR_ROOT}/scripts/eval/eval_offline_replay.py" \
    ...
    --path_output="${DIR_EXPERIMENT}/data/front_weighted"

# Copy results
echo "=== Results ===" > "${DIR_EXPERIMENT}/report.md"
echo "" >> "${DIR_EXPERIMENT}/report.md"
echo '```' >> "${DIR_EXPERIMENT}/report.md"
cat "${DIR_EXPERIMENT}/data/front_weighted_results.txt" >> "${DIR_EXPERIMENT}/report.md"
echo '```' >> "${DIR_EXPERIMENT}/report.md"
...
echo "=== 07-front-weighted-loss: Done ==="
