...
class ACTSmoothPolicy(PreTrainedPolicy):
    ...
    def forward(self, batch: dict[str, Tensor]) -> tuple[Tensor, dict]:
        ...
        # Compute L1 loss with padding mask and optional front-weighting
        loss_elementwise = F.l1_loss(targets, actions_hat, reduction="none") * ~action_is_pad.unsqueeze(-1)
        if self.config.weight_front_emphasis > 0:
            # Position weights: linearly from (1 + emphasis) at start to 1.0 at end
            pos_weights = 1.0 + self.config.weight_front_emphasis * torch.linspace(
                1.0, 0.0, chunk_size, device=actions_hat.device
            )
            loss_elementwise = loss_elementwise * pos_weights[None, None, :, None]
        l1_loss = loss_elementwise.mean()
