...
class ACTSmoothPolicy(PreTrainedPolicy):
    ...
    def forward(self, batch: dict[str, Tensor]) -> tuple[Tensor, dict]:
        ...
        # Acceleration penalty: penalize second differences in predicted action sequence
        if self.config.weight_loss_accel > 0 and chunk_size >= 3:
            # actions_hat: [B, D, C, action_dim]
            accel = actions_hat[:, :, 2:, :] - 2 * actions_hat[:, :, 1:-1, :] + actions_hat[:, :, :-2, :]
            # Mask: acceleration valid only where all 3 neighboring positions are not padded
            valid_accel = ~action_is_pad[:, :, 2:] & ~action_is_pad[:, :, 1:-1] & ~action_is_pad[:, :, :-2]
            loss_accel = (accel.pow(2) * valid_accel.unsqueeze(-1)).mean()
            loss_dict["loss_accel"] = loss_accel.item()
            loss = loss + self.config.weight_loss_accel * loss_accel
