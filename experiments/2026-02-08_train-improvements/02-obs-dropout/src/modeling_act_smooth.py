...
"""ACTSmooth Policy with observation (state) dropout.

Identical to the original ACTSmooth except: during training, proprioception
(observation.state) is zeroed out with probability p_drop_obs per sample.
This forces the model to rely more on the action prefix for continuity.

Images are NOT dropped -- only state.
"""
...
class ACTSmoothPolicy(PreTrainedPolicy):
    """ACTSmooth Policy with state dropout support."""
    ...
    def forward(self, batch: dict[str, Tensor]) -> tuple[Tensor, dict]:
        ...
        targets = batch[ACTION][:, self._indices_target]
        action_is_pad = batch[ACTION_IS_PAD][:, self._indices_target]
...
class ACTSmooth(nn.Module):
    """ACTSmooth with state dropout support."""
    ...
    def forward(self, batch: dict[str, Tensor]) -> tuple[Tensor, tuple[Tensor, Tensor] | tuple[None, None]]:
        ...
        # Zero out proprioception (observation.state) for a random subset of samples.
        # This forces the model to rely on the action prefix for continuity.
        # Images are NOT dropped.
        p_drop_obs = getattr(self.config, "p_drop_obs", 0.0)
        if is_training_batch and self.training and p_drop_obs > 0 and self.config.robot_state_feature:
            mask_drop = torch.rand(batch_size, 1, device=device) < p_drop_obs
            batch = dict(batch)
            batch[OBS_STATE] = batch[OBS_STATE] * (~mask_drop).float()
